...
distribution on $\boldsymbol{\theta} := (\theta_1, \theta_2)$ can be specified in the following heirarchical way:
...
heirarchical representation above.
...
function log_posterior(theta::AbstractVector{<:AbstractFloat}, Y::Vector{<:AbstractFloat}, mu::Vector{<:AbstractFloat})
...
scatter(results.samps[2501:10000,1], results.samps[2501:10000,2], alpha = 0.4, legend = false)
```
...
we can construct through a similar heirarchical representation:
...
(up to a constant) of the posterior distribution of $\boldsymbol{\theta}$ based on the 
heirarchical representation of the Twin Banana distribution.
...
using LinearAlgebra
  ...
  Σ_I = diagm(ones(100))
    ones_N = ones(100)
function TB_log_posterior(theta::AbstractVector{<:AbstractFloat}, Y::Vector{<:AbstractFloat}, mu::Vector{<:AbstractFloat})
    m = (0.1 * (theta[1] - mu[1])^2  -  0.5 * (theta[2] - mu[2])^4 - 10 * (theta[1]- mu[1]) * (theta[2]- mu[2])) 
    ## Normal distribution for likelihood
...
is cenetered at 0, we do not have to explicitly spcecify $\mu_0$.
...
scatter(results_TB.samps[125001:500000,1], results_TB.samps[125001:500000,2], alpha = 0.4, legend = false)
```
